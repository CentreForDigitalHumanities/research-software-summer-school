...
- Tests help you think about, and make explicit, what you expect your software to do.
...
> It may seem over-cautious to verify such a simple function right after writing it. However, an early version of these course materials had the `5` and the `9` reversed, i.e., `5 / 9` instead of `9 / 5`. That mistake could slip through exactly *because the function had not been tested*. A common adagium is that *untested code is broken* and most of the time, it's true!

The above prompts in the interactive Python interpreter are basically an implicit, throwaway unit test. It is implicit because only the author can tell whether the outcome of the test was correct or not. It is throwaway, because the author will have to write these lines again after editing the function. We will turn this into an explicit and reusable unit test in two steps.
...
With [pytest][pytest], actually making this test run is as easy as running the program `pytest`:
